...
async function withClientScope(req, res, next) {
  try {
    ...
    let { data, error } = await supabase
      .from('client_members')
      .select('client_id, role, tester_acknowledged_at, tester_acknowledged_ip, user_id_uuid, clients ( id, name )')
    ...
    if (error && error.code === '42703') {
      const retry = await supabase
        .from('client_members')
        .select('client_id, role, tester_acknowledged_at, tester_acknowledged_ip, user_id, clients ( id, name )')
        .eq('user_id', userId)
        .limit(50);
      data = retry.data;
      error = retry.error;
    ...
    const memberships = rows.map(r => ({
      ...
      role: r.role || 'member',
      tester_acknowledged_at: r.tester_acknowledged_at || null,
      tester_acknowledged_ip: r.tester_acknowledged_ip || null,
